...
func (c *Client) readPump() {
	...
	for {
		...
		println("=== lendo mensamgem ===")
		var jsonBlob = []byte(string(message))
...
func (c *Client) writePump() {
	...
	for {
		...
		case message, ok := <-c.send:
			...
			var animals Animal
			err2 := json.Unmarshal(message, &animals)
			if err2 != nil {
				fmt.Println("error:", err2)
			}
			...
			if animals.Name == c.name {
				myName = "Você"
			} else {
				myName = animals.Name
			}

			w.Write([]byte(string("(" + myName + "): " + animals.Msg)))
